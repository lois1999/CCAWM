...
verbarg_train = open("verbarg_train.SRC", "w")
verbarg_dev = open("verbarg_dev.SRC", "w")
verbarg_test = open("verbarg_test.SRC", "w")

inflection_arg = {'left': ' s', 'twice': ' x', 'thrice': ' x', 'right': ' s', 'walk': ' o', 'look': ' o', 'turn': ' i', 'jump': ' o', 'run': ' o', 'and': ' a', 'after': ' a', 'around': ' s', 'opposite': ' s'}

for line in train:
    words_train = []
    for word in line.split():
        suffix = inflection_arg[word]
        words_train.append(word + suffix)
    words_train.append("\n")
    command = " ".join(words_train)
    verbarg_train.write(command)
